...
},getAdInfo:function(d,a,c){var b=this.controller.callLayerFunction("freewheel","getAdInfo",[d.getCustomId(),a,c]);
return b
...
},submitRequest:function(){this.controller.callLayerFunction("freewheel","submitRequest",[])
}});
$pdk.ns("$pdk.plugin.freewheel.admanager");
$pdk.plugin.freewheel.admanager.FlashAdManager=$pdk.extend($pdk.plugin.freewheel.admanager.NativeAdManager,{constructor:function(b,a,d){$pdk.plugin.freewheel.admanager.FlashAdManager.superclass.constructor(b,a,d);
this.ready=false;
this._callback=d;
var c=this;
this.addEventListener("urn:theplatform:freewheel:ready",function(e){if(e){c.ready=true;
c._callback.call()
}});
a.loadLayer("flash","freewheel",$pdk.scriptRoot+"/swf/FreeWheelAdManagerLoader.swf");
$pdk.ns("tv.freewheel");
...
this._callback=c
},initialize:function(b,a,d){$pdk.plugin.freewheel.admanager.JavaScriptAdManager.superclass.initialize.call(this,b,a,d);
this.proxy=b.getVideoProxy();
this.proxy.parentNode=d;
var c=this;
...
this.ready=true;
this._callback.call()
},getAdInfo:function(j,g,b){var f={};
f.slot=j;
f.adId=g;
...
}if(this.proxy.ended||Math.abs(this.proxy.currentTime-this.videoProxy.duration)<0.25){return
...
},setRequestMode:function(a){},submitRequest:function(){this._context.submitRequest()
}});
$pdk.ns("$pdk.plugin.freewheel.admanager");
$pdk.plugin.freewheel.admanager.AdManagerFactory={};
$pdk.plugin.freewheel.admanager.AdManagerFactory.create=function(c,b,a,e){var d=null;
if(c==$pdk.plugin.freewheel.admanager.AdManagerFactory.FLASH){d=new $pdk.plugin.freewheel.admanager.FlashAdManager(b,a,e)
}else{if(c==$pdk.plugin.freewheel.admanager.AdManagerFactory.HTML5){d=new $pdk.plugin.freewheel.admanager.JavaScriptAdManager(b,a,e)
}}return d
};
...
}}},mediaPlaying:function(a){if(this._active){this._controller.dispatchEvent("OnMediaPlaying",a)
...
$pdk.ns("$pdk.plugin.freewheel.slotmanager");
$pdk.plugin.freewheel.slotmanager.MultiClipSlotManager=$pdk.extend($pdk.plugin.freewheel.slotmanager.SlotManager,{impression:false,load:false,constructor:function(a,b){$pdk.plugin.freewheel.slotmanager.SlotManager.call(this,a,b);
this.controller.addEventListener("OnMediaLoadStart",this.bind(this,this._onMediaLoadStart));
...
this.playlist=null
},_onAdImpression:function(a){this.impression=true;
this.index++;
this.adId=a.adInstance?a.adInstance.getAdId():a.adId;
this._checkStart()
},_onMediaLoadStart:function(a){if(a.data.baseClip.isAd&&a.data.streamType==="empty"){this.load=true;
this.clip=a.data;
this._checkStart()
...
this.load=false;
var a=this.adManager.getAdInfo(this.slot,this.adId,null);
if(a){this.clip.title=a.title;
this.clip.baseClip.isMid=(a.type=="midroll");
this.clip.length=parseInt(a.duration,10)*1000;
this.clip.mediaLength=parseInt(a.duration,10)*1000;
this.clip.clipEnd=this.clip.length;
this.clip.baseClip.trueLength=parseInt(a.duration,10)*1000;
if(!this.clip.baseClip.contentCustomData){this.clip.baseClip.contentCustomData={}
}this.clip.baseClip.contentCustomData["fw:category"]=a.category;
this.clip.baseClip.contentCustomData["fw:subcategory"]=a.subcategory;
this.clip.baseClip.contentCustomData["fw:type"]=a.type;
this.clip.baseClip.contentCustomData["fw:customData"]=a.customData;
this.clip.baseClip.contentCustomData["fw:adurl"]=a.adURL;
this.controller.updateClip(this.clip)
}this._noopPlayer.activate();
this._noopPlayer.mediaStarts(this.clip);
this._startProgressTimer()
},_endClip:function(){this._stopProgressTimer();
this._noopPlayer.mediaEnds(this.clip);
this._noopPlayer.deactivate()
...
this.adManager=$pdk.plugin.freewheel.admanager.AdManagerFactory.create(this.runtime,this.parameters.adManagerUrl,this.controller,this.bind(this,this.onAdManagerLoaded));
this.slotManager=(this.clipPerAd?new $pdk.plugin.freewheel.slotmanager.MultiClipSlotManager(this.controller,this.adManager):new $pdk.plugin.freewheel.slotmanager.SlotManager(this.controller,this.adManager));
...
var d=null;
if(b&&this.parameters.customVideoAssetIdField){for(field in b){if(field.indexOf("$">1)){if(field.split("$")[1]==this.parameters.customVideoAssetIdField){d=b[field]
...
return true
},doSetAds:function(){if(this.adManagerError){this.controller.setAds(null);
return
...
}else{this.adManager.setRequestMode("ON_DEMAND")
}this.resize();
...
case tv.freewheel.SDK.TIME_POSITION_CLASS_OVERLAY:break
}}if(this.setAdsWaiting){this.doSetAds()
}},setDesiredBitrate:function(a){this.log("setDesiredBitrate()"+a);
if(a>0){this.bitrate=a;
if(this.adManager&&this.adManager.ready){}}},onMediaLoadStart:function(b){var a=b.data;
this.log("onMediaLoadStart() clip.streamType = "+a.streamType+" url:"+a.URL);
if(a.streamType=="empty"&&a.baseClip.guid){var c=this.adManager.getSlotByCustomId(a.baseClip.guid);
...
}else{}}},onMediaStart:function(a){this.log("onMediaStart");
if(this.adManager&&a.data&&a.data.baseClip.isAd===false&&a.data.chapter&&a.data.chapter.index===0){this.controller.mute(true);
this.controller.mute(false);
...
}},onSlotEnded:function(a){if(this.isTemporalSlot(a.slot)){this.log("Slot ended",tpConsts.INFO);
if(this._hasStarted){this._hasStarted=false;
setTimeout(this.bind(this,function(){this.slotManager.end()
}),250)
}this.hideVideoDisplayBase()
...
},onAdImpression:function(f){this.log("onAdImpression()");
