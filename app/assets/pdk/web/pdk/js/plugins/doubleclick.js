...
}this._adContainer.id=this.AD_CONTAINER_ID_PREFIX+this._controller.id;
...
},checkIMAExists:function(){return !(typeof google=="undefined"||typeof google.ima=="undefined")
...
if(!this._adDisplayContainer){if(this.useCustomPlayback()){this.log("\tusing custom playback");
this._isCustomPlayback=true;
this._videoProxy=this._controller.getVideoProxy();
this._noopPlayer.usingVideoProxy(true);
this._adDisplayContainer=new google.ima.AdDisplayContainer(this._adContainer,this._videoProxy,this._clickTrackingElement)
}else{this.log("\tusing standard playback");
this._adDisplayContainer=new google.ima.AdDisplayContainer(this._adContainer)
}}},_createCustomClickTrackingElement:function(){this._clickTrackingElement=document.createElement("div");
this._clickTrackingElement.id=this._controller.id+".clickTrackingElement";
this._clickTrackingElement.style.position="absolute";
if(this.applyFloatingControlsFix()){this.log("shutting off pointer events for click tracking for floating controls fix");
this._clickTrackingElement.style.pointerEvents="none"
}var a=document.getElementById(this._controller.id+".plugins");
a.insertBefore(this._clickTrackingElement,this._adContainer)
...
this._adsManager=new $pdk.plugin.doubleclick.AdsManager(f.getAdsManager(this._playhead,b));
if(this._adsManager){var g=(this._isMuted?0:1);
this.log("adsManagerVolumeSetting: "+g);
this._adsManager.setVolume(g);
var d=this;
this._adsManager.addEventListener(google.ima.AdEvent.Type.ALL_ADS_COMPLETED,function(h){d._onAllAdsCompleted.apply(d,arguments)
});
this._adsManager.addEventListener(google.ima.AdErrorEvent.Type.AD_ERROR,function(h){d._onAdError.apply(d,arguments)
});
this._adsManager.addEventListener(google.ima.AdEvent.Type.CONTENT_PAUSE_REQUESTED,function(h){d._onContentPauseRequested.apply(d,arguments)
});
this._adsManager.addEventListener(google.ima.AdEvent.Type.CONTENT_RESUME_REQUESTED,function(h){d._onContentResumeRequested.apply(d,arguments)
});
this._adsManager.addEventListener(google.ima.AdEvent.Type.STARTED,function(h){d._onAdStarted.apply(d,arguments)
});
this._adsManager.addEventListener(google.ima.AdEvent.Type.PAUSED,function(h){d._onAdPaused.apply(d,arguments)
});
...
this._adsManager.addEventListener(google.ima.AdEvent.Type.COMPLETE,function(h){d._onAdCompleted.apply(d,arguments)
});
this._adsManager.addEventListener(google.ima.AdEvent.Type.DURATION_CHANGED,function(h){d._onAdDurationChanged.apply(d,arguments)
});
this._adsManager.addEventListener(google.ima.AdEvent.Type.REMAINING_TIME_CHANGED,function(h){d._onAdRemainingChanged.apply(d,arguments)
});
this._adsManager.addEventListener(google.ima.AdEvent.Type.CLICK,function(h){d._onAdClicked.apply(d,arguments)
});
this._adsManager.addEventListener(google.ima.AdEvent.Type.SKIPPABLE_STATE_CHANGED,function(h){d._onAdSkippableStateChanged.apply(d,arguments)
});
this._adsManager.addEventListener(google.ima.AdEvent.Type.LOG,function(h){d.log.apply(d,arguments)
});
this._adsManager.addEventListener(google.ima.AdEvent.Type.SKIPPED,function(h){d._onAdSkipped.apply(d,arguments)
});
...
}else{this._strategy=new $pdk.plugin.doubleclick.VastStrategy(function(h){d._requestAds.apply(d,arguments)
},this._adsLoader,this._controller)
...
if(this.isHtml5Video()&&this.isIE()){this.log("We don't load ads for IMA HTML SDK in Internet Explorer because it is not supported.");
return
...
if(this._clickTrackingElement){this._clickTrackingElement.style.left=a.x+"px";
this._clickTrackingElement.style.top=a.y+"px";
this._clickTrackingElement.style.width=a.width+"px";
this._clickTrackingElement.style.height=a.height+"px"
}this._positionFloatingControlsFixLayer()
...
},isIE:function(){var a=($pdk.isIE||!!window.MSInputMethodContext);
this.log("isIE: "+a);
return a
},applyFloatingControlsFix:function(){return !!(navigator.userAgent.match(/iPad/i)&&this._floatRegionId)
...
if(this.isAd(a.baseClip)){if(this.shouldBypassAdBreak(a)){this.warn("Bypassing this ad break.");
this._bypassAdBreak();
return true
...
}else{if(!this._adsRequestInProgress){this._requestAds(a.baseClip.URL)
...
if(this.isHtml5Video()){if(this.isIE()){b=true;
this.log("Bypass reason: IMA3 HTML5 does not support Internet Explorer.")
...
},_beginAdExperience:function(a){this.log("beginAdExperience");
this.stopProgressTimer();
this._slotPending=false;
...
if(!this._strategy.playNextPod()){this._bypassAdBreak()
}}else{if(this._adsErrorForNextCheckAd){this.warn("Due to error, bailing on ad break.");
this._controller.setAds(null);
this._adsErrorForNextCheckAd=false
...
var c={baseClips:[],clips:[],globalDataType:"com.theplatform.pdk.data::Playlist"};
a.setAds(c)
},1)
...
if(this._clickTrackingElement){this._clickTrackingElement.style.display="block"
...
if(a.getAd().isLinear()&&this._strategy){this.stopProgressTimer();
...
}if(this._clickTrackingElement){this._clickTrackingElement.style.display="none"
...
}},_onAllAdsCompleted:function(a){this.log("onAllAdsCompleted")
...
$pdk.plugin.doubleclick.VastStrategy=$pdk.extend(function(){},{_adsManager:null,_controller:null,_slotPlayRequested:false,_ad:null,_adLoadedReady:false,_preparePodCallback:null,_firstPod:true,_adLoadedListener:null,_adBreakReadyListener:null,_adsLoader:null,_isOverlay:false,constructor:function(c,a,b){this._adsLoader=a;
this._controller=b;
...
},setAdsManager:function(b){var a=this;
this._adsManager=b;
this._adsManager.addEventListener(google.ima.AdEvent.Type.LOADED,this._adLoadedListener);
this._adsManager.addEventListener(google.ima.AdEvent.Type.AD_BREAK_READY,this._adBreakReadyListener)
