...
$pdk.plugin.mvpdPicker = $pdk.extend(function(){},
{
	...
	initialize:function(lo)
	{
		...
		window.tpUseCustomMVPDPicker = (lo.vars["useCustomMVPDPicker"] ? lo.vars["useCustomMVPDPicker"].toLowerCase() == "true" : false);

		try {
			adobePassShim.parseResourceIds(lo.vars["resourceIdMap"]);
			tpDebug(this.resourceIds, "mvpdPicker", "mvpdPicker");
		}
		catch (error) {
			tpDebug("Error: malformed resource id map!");
		}

		if (window.adobePassProvider)
		{
			this.provider = window.adobePassProvider;
		}

		if (window.adobePassResourceIds && this.provider)
		{
			tpDebug("initialize(): Setting ResourceId");
			adobePassShim.setResourceId(window.adobePassResourceIds[this.provider]);
		}

        tpDebug("*** mvpdPicker plugin 	! ***");
	},
...
if (!window.tpUseCustomMVPDPicker) {
...
var adobePassShim = {
    ...
    init: function()
    {
        ...
        //$pdk.controller.addEventListener("OnSetRelease", adobePassShim.onSetRelease);
    },

	parseResourceIds: function(map)
	{
		tpDebug("parseResourceIds(): " + map);
		...
		{
			tpDebug("parseResourceIds(): Setting ResourceId");
    ...
    setResourceId: function(id){
        if(this.isSwf){
            ...
            tpDebug("adobe pass swf:" + this.adobePassSwf + " swf:" + swf);
...
{
	tpDebug('onTrackingData: ' + trackingEventType);

    if (trackingEventType == "authenticationDetection" && trackingData[1]) {
		tpDebug("Provider: " + trackingData[1]);

		if (window.adobePassResourceIds && window.adobePassResourceIds[trackingData[1]])
		{
			tpDebug("setting ResourceId: " + adobePassResourceIds[trackingData[1]]);
	        adobePassShim.setResourceId(adobePassResourceIds[trackingData[1]]);
		}
		else
		{
			tpDebug("no ResourceId Map, saving provider: " + trackingData[1]);
			tpDebug(window.adobePassResourceIds)
			tpDebug(window.adobePassResourceIds[trackingData[1]])
...
{
	tpDebug('onSetProvider: ' + providerId);
...
function displayMVPDPicker(arr)
{
	if (window.tpUseCustomMVPDPicker)
	{
		$pdk.controller.dispatchEvent("OnShowProviderPicker", {providers: arr});
		return;
	}
...
try {
	$pdk.controller.plugInLoaded(new $pdk.plugin.mvpdPicker(), null);
}
catch (error)
{
