...
//            _controller.addEventListener(PdkEvent.OnMediaPlaying, onMediaPlaying);
        ...
        private function onAdsManagerLoaded(e:AdsManagerLoadedEvent):void
        {
            ...
            if (_adsManager)
            {
                ...
                _adsManager.addEventListener(AdEvent.REMAINING_TIME_CHANGED, onAdRemainingChanged);
...
//            log(l);
        ...
        private function logAreas():void
        {
            var oa:Rectangle = _controller.getOverlayArea();
            var ma:Rectangle = _controller.getMediaArea();

            log("overlay area: "+oa.x+","+oa.y+","+oa.width+","+oa.height);
            log("media area: "+ma.x+","+ma.y+","+ma.width+","+ma.height);
        }
...
//                logAreas();
        ...
        {
    //        if(e.data && e.data.newFileInfo && e.data.newFileInfo.bitrate)
...
//            }
        ...
        public function checkAd(clip:Clip):Boolean
        {
            if (isAd(clip.baseClip))
            {
                ...
                else if(!_adsRequestInProgress) // PDK-10510: if we get an error, we need to make a fresh request on next checkAd since strategy destroyed
...
//            _controller.dispatchEvent(new PdkEvent(PdkEvent.OnOverlayStart,null));
        ...
        private function onAdRemainingChanged(e:AdEvent):void
        {
            log("IMA Event:"+e.type+":"+_adsManager.remainingTime);
        }
...
//                }
        ...
        {
            log("IMA Event:"+e.type+": "+ e.adData.toString());
        }
...
//            try{log("- ad linear: "+ e.ad.linear+", adsMgr linear: "+_adsManager.linear)}catch(e:Error){log("e.ad is null")}
            if(_strategy)
            {
                _strategy.contentResumeRequested();
            }
        }
...
//            try{log("- ad linear: "+ e.ad.linear+", adsMgr linear: "+_adsManager.linear)}catch(e:Error){log("e.ad is null")}
        ...
        private function updateAdPlaylist(ad:Ad):void
        {
            ...
            {
                _controller.trace("updating ad Playlist", "Doubleclick", Debug.WARN);
                for (var i:Number = 1; i < ad.adPodInfo.totalAds; i++)
                {
                    _playlist.addClip(createAdClip("DFP:"+ (ad.adPodInfo.podIndex+1) + ":" + ad.adPodInfo.adPosition));
                    _playlist.reset()
                }
            }
        }
